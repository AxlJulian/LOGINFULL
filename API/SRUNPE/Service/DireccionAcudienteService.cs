...
internal sealed class DireccionAcudienteService : IDireccionAcudienteService
{
    ...
    public DireccionAcudienteDto CreateDireccionAcudiente(DireccionAcudienteForCreationDto direccionAcudienteDto)
    {
        var existingDireccion = _repository.DireccionAcudiente.FindByCondition(d => d.AcudienteId == direccionAcudienteDto.AcudienteId).FirstOrDefault();
        if (existingDireccion != null)
        {
            // Actualizar la dirección existente
            existingDireccion.Calle = direccionAcudienteDto.Calle;
            existingDireccion.ColoniaBarrio = direccionAcudienteDto.ColoniaBarrio;
            existingDireccion.CiudadLocalidad = direccionAcudienteDto.CiudadLocalidad;
            existingDireccion.CodigoPostal = direccionAcudienteDto.CodigoPostal;
            existingDireccion.EstadoProvincia = direccionAcudienteDto.EstadoProvincia;
            existingDireccion.Pais = direccionAcudienteDto.Pais;

            _repository.DireccionAcudiente.UpdateDireccionAcudiente(existingDireccion);
            _repository.Save();
            return _mapper.Map<DireccionAcudienteDto>(existingDireccion);
        }
        else
        {
            // Insertar la nueva dirección
            var direccionAcudienteEntity = _mapper.Map<DireccionAcudiente>(direccionAcudienteDto);
            _repository.DireccionAcudiente.CreateDireccionAcudiente(direccionAcudienteEntity);
            _repository.Save();
            return _mapper.Map<DireccionAcudienteDto>(direccionAcudienteEntity);
        }
    }
